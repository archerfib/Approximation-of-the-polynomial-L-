...
def loss(a_list):  # 定义损失函数L∞
    g_list = np.dot(X_part, a_list)
    return np.max(np.abs(y_list - g_list))
...
while 1:
    ...
    for i in range(1, 11):
        ...
        X_part = X[...,10-i:]
        Var=sigma**2   #通过修改Var的计算公式可改变拟合的精度，但追求拟合的精度可能导致程序运行效率低下
        ii=0
        ...
        print('%d次多项式为：\n'%i,poly)
        print('%d次多项式拟合的L∞距离为:' % i,res.fun)
        # 作图
        x=np.linspace(0, 10, 1000)
        plt.scatter(x_list,y_list)
        plt.plot(x,np.polyval(poly, x))
        plt.title('L∞ regression: %d degree polynomial'%i)
        plt.show()
